...
def make_train(config):
    ...
    def train(rng):
        ...
        image, feats = process_world_state(initial_world_state)
        ...
        def _env_step(runner_state):
            ...
            world_image = world_image.reshape((config["NUM_ACTORS"], *world_image.shape[-3:]))
            cr_in = (
                (world_image[None, :], world_feats[None, :]),
                last_done[np.newaxis, :],
            )
            cr_hstate, value = critic.apply(train_states[1].params, hstates[1], cr_in)
        ...
        advantages, targets = scan_adv(traj_batch, last_val, config["GAMMA"], config["GAE_LAMBDA"])
