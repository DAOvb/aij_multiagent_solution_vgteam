...
class CentralCritic(nn.Module):
    ...
    def __call__(self, x):
        dtype = jnp.float32
        x = x["image"]
        p_x1, p_x2, p_x3 = x["wealth"], x["has_resource"], x["has_trash"]
...
def policy_action(
    ...
    out = _policy_action(apply_fn, params, observation)
    return out

def critic_step(train_state: train_state.TrainState,
    observation: dict["str", np.ndarray],
    rng_key: jax.random.PRNGKey,):
    model_output = policy_action(train_state.apply_fn, train_state.params, observation)
    agent_values = {}
...
def get_experience(
    ...
    for step in tqdm(range(num_steps + 1), desc="collecting exp"):
        ...
        for i, world_state in enumerate(world_states):
            value = critic_step(critic_train_state, world_state, keys[i])
